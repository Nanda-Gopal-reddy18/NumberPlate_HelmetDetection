...
class ViolationEngine:
    ...
    def helmet_matches_rider(self, helmet: Detection, person: Detection) -> bool:
        ...
        result = center_in_head and helmet_cover_ratio >= 0.25
        print(f"  Helmet matching: person={person.box}, helmet={helmet.box}")
        print(f"    head_box={head_box}, intersection_area={inter}, helmet_box_area={box_area(helmet.box)}")
        print(f"    helmet_cover_ratio={helmet_cover_ratio:.3f}, center_in_head={center_in_head}, match={result}")
        return result
    ...
    def build_challan(
        ...
        print(f"DEBUG build_challan: no_helmet_riders={no_helmet_riders}, country={country}, plate_text='{plate_text}'")
        if no_helmet_riders <= 0:
            print(f"DEBUG: No no-helmet violations found, returning None")
            return None

        print(f"DEBUG: Creating challan for {no_helmet_riders} no-helmet riders")
        if country not in CHALLAN_RULES:
            print(f"DEBUG: Country '{country}' not in CHALLAN_RULES, using default India rules")
        ...
        print(f"DEBUG: Challan created: {challan}")
        return challan
    ...
    def analyze(self, frame: np.ndarray, country: str) -> Dict[str, object]:
        ...
        if persons:
            for i, p in enumerate(persons):
                print(f"  Person {i}: confidence={p.confidence:.2f}, box={p.box}")
        print(f"Bikes detected: {len(bikes)}")
        if bikes:
            for i, b in enumerate(bikes):
                print(f"  Bike {i}: confidence={b.confidence:.2f}, box={b.box}")
        print(f"Riders matched: {len(riders)}")
        if riders:
            for i, r in enumerate(riders):
                print(f"  Rider {i}: person={r['person'].label}, bike={r['bike'].label}")
        print(f"Helmets detected: {len(helmets)}")
        if helmets:
            for i, h in enumerate(helmets):
                print(f"  Helmet {i}: confidence={h.confidence:.2f}, box={h.box}")
        print(f"Helmeted riders (indices): {helmeted_riders}")
        print(f"Riders without helmet indices: {no_helmet_indices}")
        print(f"Count of riders without helmet: {len(no_helmet_indices)}")
        print(f"{'='*60}\n")
    ...
    def analyze_plate(self, frame: np.ndarray, country: str) -> Dict[str, object]:
        if not self.models_loaded:
            self.load_models()
        ...
        # Build plate-specific challan if plate is missing or invalid
        challan = None
        if not plate_detection or not plate_valid:
            rule = CHALLAN_RULES.get(country, CHALLAN_RULES["India"])
            fine_amount = rule.get("helmet_no_rider", 1000)
            offense = "No License Plate Detected" if not plate_detection else "Invalid License Plate Format"
            challan = {
                "offense": offense,
                "offender_count": "1",
                "fine_unit": f"{rule['currency']} {fine_amount}",
                "fine_total": f"{rule['currency']} {fine_amount}",
                "plate_number": plate_text if plate_text else "Not detected",
                "notes": rule["notes"],
                "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
            }

        vehicle_details = self.infer_vehicle_details(plate_text, country, 0, 0, plate_valid)
        ...
        summary = f"Plate Mode | Valid: {'Yes' if plate_valid else 'No'}"
        draw_text_label(
            annotated,
            summary,
            (12, annotated.shape[0] - 10),
