...
def send_violation_email(to_email, subject, body, root=None):
    ...
    try:
        # Use SSL context to avoid protocol violations
        import ssl
        ...
        # Don't show error popup - email is non-critical, challan generation is what matters
        # The challan will still be generated and displayed in the report
...
class HelmetDetectionApp:
    ...
    def analyze_image(self) -> None:
        ...
        try:
            ...
            # 🔥 identify image name for email
            image_name = self.selected_image.stem  # e.g., bikes1 → IMG_1

            # map image → email (non-critical feature)
            ...
            if user:
                # check violation - only email if we have violations
                if result["no_helmet_count"] > 0 or not result["plate_detected"]:
                    try:
                        ...
                        # Email is non-critical - continue even if it fails
